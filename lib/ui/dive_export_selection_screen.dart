...
class _DiveExportSelectionScreenState extends State<DiveExportSelectionScreen> {
  ...
  bool _isExportable(Dive dive) =>
      SsiQrPayloadBuilder.unexportableReason(dive) == null;
  ...
  void _toggleDay(_DiveDay day, bool select) => setState(() {
    for (final dive in day.dives.where(_isExportable)) {
      if (select) {
        _selected.add(dive.id);
      } else {
        _selected.remove(dive.id);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Mehrere exportieren')),
      body: ListView(
          ...
          for (final day in _days) ...[
            _DayHeader(
              ...
              selectedCount: day.dives
                  ...
                  .length,
              exportableCount: day.dives.where(_isExportable).length,
            ...
            for (final dive in day.dives) ...[
              _SelectableDive(
                ...
                reason: SsiQrPayloadBuilder.unexportableReason(dive),
        ...
        child: Padding(
          ...
          child: FilledButton.icon(
            ...
            label: Text(switch (count) {
              0 => 'Tauchgänge auswählen',
              1 => '1 Tauchgang als QR-Code',
              _ => '$count Tauchgänge als QR-Codes',
      ...
      child: Row(
        children: [
          Expanded(
            child: SectionHeader(
              title:
                  '${Fmt.weekday(day.date)}, ${Fmt.date(day.date)} · '
                  '${day.dives.length} TG',
            ),
          ),
          if (exportableCount > 0)
            TextButton(
              ...
              child: Text(allSelected ? 'Keinen' : 'Ganzer Tauchtag'),
            ),
        ],
      ),
    );
